...
async def test_judge_case_clamps_out_of_range_score(
    ...
    mock_llm_client.chat_completion.return_value = {
        "choices": [
            {"message": {"content": '{"score": 9, "rationale": "Way off scale."}'}}
        ]
    }
...
async def test_judge_case_falls_back_on_invalid_json(
    ...
    mock_llm_client.chat_completion.return_value = {
        "choices": [{"message": {"content": "I refuse to output JSON today."}}]
    }
...
async def test_judge_case_preserves_raw_answer_even_when_empty(
    ...
    mock_llm_client.chat_completion.return_value = {
        "choices": [
            {"message": {"content": '{"score": 1, "rationale": "No answer provided."}'}}
        ]
    }
